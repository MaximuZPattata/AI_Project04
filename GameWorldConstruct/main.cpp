...
extern void UpdateAIPositions();
extern void CreatePathPatternForEnemies();
extern void CreateDangerZones(cControlGameEngine& gameEngine);
extern void InitializePlayer(); 
extern void UpdatePlayerPosition(cControlGameEngine& gameEngine, GLFWwindow* window);

extern std::vector < sPath > pathList;
extern std::vector < sDangerZone > dangerZoneList;
...
int main()
{
    ...
    CreatePathPatternForEnemies();
    CreateDangerZones(gameEngine);
    InitializePlayer();

    //----------------------------Initialize Camera-----------------------------------------

    gameEngine.cameraHeight = playerAttributes.playerPosition.y + 10.f;
    gameEngine.cameraDistance = 35.f;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for(int index = 0; index < enemyDetailsList.size(); index++)
            enemyDetailsList[index]->FollowPath(gameEngine, pathList, dangerZoneList, playerAttributes);

       //------------------------Update Player----------------------------------------------

        gameEngine.UpdateThirdPersonCamera(playerAttributes.playerPosition, playerAttributes.playerYaw, playerAttributes.playerPitch);

        UpdatePlayerPosition(gameEngine, window);
