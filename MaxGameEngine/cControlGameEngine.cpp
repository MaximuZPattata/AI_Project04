...
void cControlGameEngine::UpdateThirdPersonCamera(const glm::vec3& playerPosition, float playerYaw, float playerPitch)
{
    if (!this->bFreeFlowCamera)
    {
        ...
        this->cameraTarget = playerPosition;
    }
}
...
void cControlGameEngine::LoadModelsInto3DSpace(std::string filePath, std::string modelName, float initial_x, float initial_y, float initial_z)
{
    ...
    if (!this->IsMeshFilePathAlreadyExisting(filePath))
    {
        sModelDrawInfo* newModel = new sModelDrawInfo();

        if (!this->mVAOManager->LoadModelIntoVAO(modelName, filePath, *newModel, shaderProgramID))
        {
            std::cout << "ERROR : UNABLE TO LOAD MODEL - MODEL NAME : " << modelName << std::endl;
            return;
        }
    ...
    //-----------------Creating new sModelDrawInfo instance without fecthing the vertex and index from file-------------------------

    else
    {
        std::string fullFilePath = this->mVAOManager->getBasePath() + "/" + filePath;

        for (sModelDrawInfo* existingModel : this->MeshDrawInfoList)
        {
            if (fullFilePath == existingModel->meshFileName)
            {
                sModelDrawInfo* newModel = new sModelDrawInfo(*existingModel);

                this->mVAOManager->LoadModelIntoVAOWithExistingModelPath(modelName, filePath, *newModel, shaderProgramID);
    ...
    this->MeshFilePathList.push_back(filePath);
}
